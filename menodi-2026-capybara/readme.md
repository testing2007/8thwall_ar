...
当前已启用 `meno` 和 `poko`。应用启动时会把两份 Image Target 数据注册给 8th Wall；识别到某个目标后，才会加载该角色对应的 GLB、MP3 和 SRT。加载结果会缓存，重复扫描不会再次下载。

角色与资源的唯一配置入口是 `src/characters.js`。识别事件使用目标 JSON 内的 `name` 自动匹配，不要另写一份容易失配的 `TARGET_NAME`。
...
```text
image-targets/bobo.json
src/assets/bobo.glb
src/assets/bobo-voice.mp3
src/assets/bobo-voice.srt
```

然后在 `src/characters.js` 顶部引入目标，并在 `CHARACTERS` 中增加配置：
...
`riko` 同理。不要在资源缺失时提前添加 `require()`，因为 webpack 会把缺少的资源视为构建错误。添加后运行：

```bash
npm run build
```

本地预览使用 8th Wall Desktop App 提供的 `http://localhost:58000/`。
